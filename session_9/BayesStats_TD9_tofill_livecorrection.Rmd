...
for (i in 1:R) {
   ...
   for (j in 1:T) {
        ...
        logit(p[i,j]) <- alpha.p + beta1.p*DATES[i,j] + beta2.p*pow(DATES[i,j],2) + beta3.p*HOURS[i,j] + beta4.p*pow(HOURS[i,j],2)
      } #j
   } #i

# Derived quantities
occ.fs <- sum(z[])                           # Number of occupied sites
mean.p <- exp(alpha.p)/(1+exp(alpha.p)) # or mean(p[,]) # Sort of average detection
}
",fill = TRUE)
