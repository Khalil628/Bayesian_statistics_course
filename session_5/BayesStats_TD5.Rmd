...
str(bdatal <- list(LC=LC[sel,], M = newM, T = ncol(LC[sel,]), yr = year - mean(year), elev = elev.sc[sel,],
    forest = forest.sc[sel,], date = date.sc[sel,], dur = dur.sc[sel,],
...
cat(file = "model_lmm4.txt","
    ...
    # Year - fixed (slope parameter)
    gamma ~ dnorm(0,1) #or dnorm(0,0.1)
...
* date has likely no strong effect
* duration a little bit (logically)
* two or three surveys changes little the result
...
Yes, using [shrinkage](https://en.wikipedia.org/wiki/Shrinkage_(statistics)). 
...
cat(file = "model_lmm5.txt","
    ...
    # Site effect on abundance
    for(i in 1:M){
      site[i] ~ dnorm(0, tau.site) # Random site effects
      }
    tau.site <- pow(sd.site, -2)
    sd.site ~ dunif(0, 1) #exp(1)

    # Residual year effect on abundance
    for(t in 1:T){
      year[t] ~ dnorm(0, tau.year) # Random year effects
      }
    tau.year <- pow(sd.year, -2)
    ...
    # Hyperprior on the trend
    mu.gamma ~ dnorm(0,1) 
    sd.gamma ~ dexp(1) # not too broad (instead  #exp(1))
    tau.gamma <- pow(sd.gamma,-2)
    
    # Priors on the trend 
    for(i in 1:M){
      gamma[i] ~ dnorm(mu.gamma, tau.gamma) # Random slopes
      }
...
cat(file = "model_lmm5_bis.txt","
    ...
    # Hyperprior on the trend
    # mu.gamma ~ dnorm(0,1) 
    # sd.gamma ~ dunif(0, 2) # not too broad
    # tau.gamma <- pow(sd.gamma,-2)
    # Priors on the trend 
    for(i in 1:M){
      gamma[i] ~ dnorm(0, 0.000001) # Random slope effects
      }
...
We found logical and expected results! 
