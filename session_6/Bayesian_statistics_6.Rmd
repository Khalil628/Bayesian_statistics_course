...
$\mathbb{E}(Y_i) = \mu = \frac{\alpha}{\beta} = \frac{r(1-p)}{p}$ and we can show that $\mathbb{V}(Y_i) = \mu + \mu^2/r$. 

## Poisson--Log-Normal

$Y_i | \epsilon_i \sim \mathcal{P}(a + b x_i + \epsilon_i)$ with $\epsilon_i \sim \mathcal{N}(0,\sigma^2)$ for regression

$Y_i | \epsilon_i \sim \mathcal{P}(\alpha_{j[i]} + \epsilon_i)$ with $\epsilon_i \sim \mathcal{N}(0,\sigma^2)$ for ANOVA
...
Each samples contains DNA sequence counts for 15 species. The total number of counts are determined by the sequencing depth -- not how much DNA we have.
