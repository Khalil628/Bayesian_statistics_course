...
98% of the variation is now due to the sites. What can that large change be due to? 
- The fact that we selected previously only rich sites
- The fact that we use now a different model that sucks up residual variation with the Poisson distribution

Let's try to select only the previous 97 sites to compare.  
...
Now we've got 78% of the variance explained by the site. Thus while the Poisson model does "absord" some of the residual variation, most of the reduction in the amount of residual variation and increase in the site-level variance is due to the fact that we have added many "poor" sites. This begs the question of whether the fixed effects that we evidenced previously also hold whenever we consider the full dataset under a Poisson model. 
