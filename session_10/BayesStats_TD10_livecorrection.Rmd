...
```{r}
datareg = data.frame(y,x1,x2,x3,x4)
BF  = regressionBF(y ~ .,data=datareg,whichModels = 'bottom') ## Just one covariate added w.r.t. constant
BF
## Comparison x1 vs x1+x2 to add. 
marginaldens_x1only  = lmBF(y ~ x1,data=datareg) ## Just one covariate added w.r.t. constant
marginaldens_x1andx2  = lmBF(y ~ x1+x2,data=datareg)
BF = marginaldens_x1andx2 / marginaldens_x1only
BF  # The model with x2 is not preferred (even though x2 has a true effect) in this universe where variable x3 is not included. Keep in mind, x2 has a small effect. 
```
...
model {
  # Priors
  for (i in 1:4){beta[i] ~ dnorm(0,0.1)}
  sigma ~ dunif(0,1)
  tau<-pow(sigma,-2)
...
model {
  # Priors
  for (i in 1:3){beta[i] ~ dnorm(0,0.1)}
  sigma ~ dunif(0,1)
  tau<-pow(sigma,-2)
...
# Inits function
inits1 <- function(){list(beta0 = rnorm(1,0, 1),
                         beta = rnorm(4,0,1))}
inits2 <- function(){list(beta0 = rnorm(1,0, 1),
                         beta = rnorm(3,0,1))}
...
Objective: Fit in JAGS the Gompertz model corresponding to this simulated dataset, and then fit the von Bertalanffy model. 
...
Which model is selected? First model, which is the truth. 
...
b= 1/10
timeindex=1:100
vonbert_growth = a*(1-exp(-b*timeindex)) + rnorm(100,0,1)
plot(timeindex,vonbert_growth)
data = list(T=100,y=vonbert_growth)
```

Now fitting both models

```{r fitting-to-vonbertalanffy}
...
# Call JAGS, check convergence and summarize posteriors
out3 <- jags(data, inits, params, "vonbert.growth.txt", n.thin = nt, 
            n.chains = nc, n.burnin = nb, n.iter = ni)
...
Do we still get the right model?  The Gompertz model has clearly higher DIC - we still select the right model. 
